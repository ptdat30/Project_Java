...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Đặt thông báo lỗi từ server hoặc lỗi mặc định
      setError(
          err.response?.data?.message || "Đã có lỗi xảy ra khi đăng nhập."
      );
    } finally {
      setLoading(false); // Luôn kết thúc loading sau khi request hoàn thành
    }
  };

  const handleGoogleSuccess = async (credentialResponse) => {
    setError(""); // Xóa lỗi cũ
    setLoading(true); // Bắt đầu loading cho Google login
    try {
      const response = await axios.post(
  ...
  const handleGoogleError = () => {
    ...
    // Không cần setLoading(false) ở đây vì đây là lỗi từ client Google OAuth, không phải từ API của bạn.
    // Nếu bạn muốn hiển thị spinner trong trường hợp này, bạn có thể cân nhắc.
  };
